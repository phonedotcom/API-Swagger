...
public class DevicesApiTest {
    ...
    public void createGetReplaceDeleteDevice() throws ApiException {
        ...
        String name = "Geordi-" + UUID.randomUUID().toString();
		data.setName(name);

		// TODO: [API Error] Create call: Response 422 Unprocessable entity even with very basic data info
        // If the device with the specified name was already created, you'll get the above exception
        // Since the device can't be deleted according to API, this test may execute successfully only once with a fixed name
        // Using UUID makes the device name random with very small probability to hit previous device name
        ...
        // Replace
        String newDeviceName = name + "2";
        data.setName(newDeviceName);
        ...
        assertEquals(newDeviceName, responseGetAfterReplace.getName());
    }
