...
@interface SWGMenusApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation SWGMenusApi

NSString* kSWGMenusApiErrorDomain = @"SWGMenusApiErrorDomain";
NSInteger kSWGMenusApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) createAccountMenuWithAccountId: (NSNumber*) accountId
    ...
    if (accountId == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kSWGMenusApiErrorDomain code:kSWGMenusApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"SWGMenuFull*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((SWGMenuFull*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) deleteAccountMenuWithAccountId: (NSNumber*) accountId
    ...
    if (menuId == nil) {
        NSParameterAssert(menuId);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"menuId"] };
            NSError* error = [NSError errorWithDomain:kSWGMenusApiErrorDomain code:kSWGMenusApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
...
-(NSURLSessionTask*) listAccountMenusWithAccountId: (NSNumber*) accountId
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"SWGListMenus*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((SWGListMenus*)data, error);
                                }
                            }];
}
